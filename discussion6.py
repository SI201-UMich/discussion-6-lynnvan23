...
###############################################################################
    def load_results(self, table):
        ...
        for list in table[1:]:
           horse_name = list[0]
           results_dict[horse_name] = {}


           for i in range(1, len(list)):
               race_name = headers[i]
               race_time = float(list[i])


               results_dict[horse_name][race_name] = race_time
        return results_dict
...
###############################################################################
    ...
    def horse_fastest_race(self, horse):
        ...
        if horse not in self.results_dict:
            return (None, 999.9)
        
        horse_name = self.results_dict[horse]

        for race, time in horse_name.items():
